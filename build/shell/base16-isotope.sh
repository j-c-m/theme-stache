#!/usr/bin/env bash

# Source:   base16
# Theme:    Isotope
# Author:   Jan T. Sott
# Variant:  dark

# Exit if not on a tty

if [[ ! -t 0 ]]; then
    exit 0
fi

print_osc4() {
    local color="$1"
    local hexterm="$2"

    printf "\033]4;%d;rgb:%s\033\\" "$color" "$hexterm"
}

print_osc_rgb() {
    local osc="$1"
    local hexterm="$2"

    printf "\033]%d;rgb:%s\033\\" "$osc" "$hexterm"
}

print_linux() {
    local color="$1"
    local hex="$2"

    printf "\033]P%x%s" "$color" ${hex:#\#}
}

do_osc() {
    print_osc4 0 "00/00/00"
    print_osc4 1 "ff/00/00"
    print_osc4 2 "33/ff/00"
    print_osc4 3 "ff/00/99"
    print_osc4 4 "00/66/ff"
    print_osc4 5 "cc/00/ff"
    print_osc4 6 "00/ff/ff"
    print_osc4 7 "d0/d0/d0"
    print_osc4 8 "80/80/80"
    print_osc4 9 "ff/00/00"
    print_osc4 10 "33/ff/00"
    print_osc4 11 "ff/00/99"
    print_osc4 12 "00/66/ff"
    print_osc4 13 "cc/00/ff"
    print_osc4 14 "00/ff/ff"
    print_osc4 15 "ff/ff/ff"

    print_osc_rgb 10 "d0/d0/d0"
    print_osc_rgb 11 "00/00/00"
    print_osc_rgb 12 "d0/d0/d0"
    print_osc_rgb 17 "e0/e0/e0"
    print_osc_rgb 19 "40/40/40"
}

do_linux() {
    print_linux 0 "#000000"
    print_linux 1 "#ff0000"
    print_linux 2 "#33ff00"
    print_linux 3 "#ff0099"
    print_linux 4 "#0066ff"
    print_linux 5 "#cc00ff"
    print_linux 6 "#00ffff"
    print_linux 7 "#d0d0d0"
    print_linux 8 "#808080"
    print_linux 9 "#ff0000"
    print_linux 10 "#33ff00"
    print_linux 11 "#ff0099"
    print_linux 12 "#0066ff"
    print_linux 13 "#cc00ff"
    print_linux 14 "#00ffff"
    print_linux 15 "#ffffff"
}

case "$TERM" in
    linux*)
        do_linux
        ;;
    *)
        do_osc
        ;;
esac

unset -f print_osc4
unset -f print_osc_rgb
unset -f print_linux
unset -f do_osc
unset -f do_linux
