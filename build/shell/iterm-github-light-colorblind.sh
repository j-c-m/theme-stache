#!/usr/bin/env bash

# Source:   iterm
# Theme:    GitHub-Light-Colorblind
# Author:   unknown
# Variant:  light

# Exit if not on a tty

if [[ ! -t 0 ]]; then
    exit 0
fi

print_osc4() {
    local color="$1"
    local hexterm="$2"

    printf "\033]4;%d;rgb:%s\033\\" "$color" "$hexterm"
}

print_osc_rgb() {
    local osc="$1"
    local hexterm="$2"

    printf "\033]%d;rgb:%s\033\\" "$osc" "$hexterm"
}

print_linux() {
    local color="$1"
    local hex="$2"

    printf "\033]P%x%s" "$color" ${hex:#\#}
}

do_osc() {
    print_osc4 0 "24/29/2f"
    print_osc4 1 "b3/59/00"
    print_osc4 2 "05/50/ae"
    print_osc4 3 "4d/2d/00"
    print_osc4 4 "09/69/da"
    print_osc4 5 "82/50/df"
    print_osc4 6 "1b/7c/83"
    print_osc4 7 "6e/77/81"
    print_osc4 8 "57/60/6a"
    print_osc4 9 "8a/46/00"
    print_osc4 10 "09/69/da"
    print_osc4 11 "63/3c/01"
    print_osc4 12 "21/8b/ff"
    print_osc4 13 "a4/75/f9"
    print_osc4 14 "31/92/aa"
    print_osc4 15 "8c/95/9f"

    print_osc_rgb 10 "24/29/2f"
    print_osc_rgb 11 "ff/ff/ff"
    print_osc_rgb 12 "09/69/da"
    print_osc_rgb 17 "24/29/2f"
    print_osc_rgb 19 "ff/ff/ff"
}

do_linux() {
    print_linux 0 "#24292f"
    print_linux 1 "#b35900"
    print_linux 2 "#0550ae"
    print_linux 3 "#4d2d00"
    print_linux 4 "#0969da"
    print_linux 5 "#8250df"
    print_linux 6 "#1b7c83"
    print_linux 7 "#24292f"
    print_linux 8 "#57606a"
    print_linux 9 "#8a4600"
    print_linux 10 "#0969da"
    print_linux 11 "#633c01"
    print_linux 12 "#218bff"
    print_linux 13 "#a475f9"
    print_linux 14 "#3192aa"
    print_linux 15 "#8c959f"
}

case "$TERM" in
    linux*)
        do_linux
        ;;
    *)
        do_osc
        ;;
esac

unset -f print_osc4
unset -f print_osc_rgb
unset -f print_linux
unset -f do_osc
unset -f do_linux
